function varargout = MergeFilesGui(varargin)
% MERGEFILESGUI MATLAB code for MergeFilesGui.fig
%      MERGEFILESGUI, by itself, creates a new MERGEFILESGUI or raises the existing
%      singleton*.
%
%      H = MERGEFILESGUI returns the handle to a new MERGEFILESGUI or the handle to
%      the existing singleton*.
%
%      MERGEFILESGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MERGEFILESGUI.M with the given input arguments.
%
%      MERGEFILESGUI('Property','Value',...) creates a new MERGEFILESGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before MergeFilesGui_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to MergeFilesGui_OpeningFcn via varargin.
%
...
% Last Modified by GUIDE v2.5 17-Dec-2015 17:33:34

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @MergeFilesGui_OpeningFcn, ...
                   'gui_OutputFcn',  @MergeFilesGui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before MergeFilesGui is made visible.
function MergeFilesGui_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to MergeFilesGui (see VARARGIN)

% Choose default command line output for MergeFilesGui
handles.output = '';
...
% UIWAIT makes MergeFilesGui wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = MergeFilesGui_OutputFcn(hObject, eventdata, handles) 
