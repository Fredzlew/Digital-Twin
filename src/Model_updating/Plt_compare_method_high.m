...
% Costfunctions
dataSSIFreq = load('.\Costfunctions\data_updated_par\SSIfreq_high.mat'); % SSI FREQ
K_SSI_freq = dataSSIFreq.k;

dataSSImode = load('.\Costfunctions\data_updated_par\SSImode_high.mat'); % SSI modes
K_SSI_mode = dataSSImode.k;

dataSSImodemac = load('.\Costfunctions\data_updated_par\SSImode_mac_high.mat'); % SSI modes
K_SSI_mode_mac = dataSSImodemac.k;


dataSSIFreqmode = load('.\Costfunctions\data_updated_par\SSIfreqmode_high.mat'); % SSI FREQ and modes
K_SSI_freq_mode = dataSSIFreqmode.k;

dataSSIfreqmodeEIL = load('.\Costfunctions\data_updated_par\SSIfreqmodeEIL_high.mat'); % SSI modes
K_SSI_freqmodeEIL = dataSSIfreqmodeEIL.k;


% Sensitivity method
dataSSIFreq_sens = load('.\Sensitivity_method\data_updated_par_sens\Eigenvalue_residual_high.mat'); % SSI FREQ
K_SSI_freq_sens = dataSSIFreq_sens.knew';

dataSSImode_sens = load('.\Sensitivity_method\data_updated_par_sens\Mode_shape_residual_high.mat'); % SSI modes
K_SSI_mode_sens = dataSSImode_sens.knew';

dataSSIFreqmode_sens = load('.\Sensitivity_method\data_updated_par_sens\Eigenvalue_Mode_shape_residual_high.mat'); % SSI modes and freq
